...
function draw () {
  ...
  let lineHeight = textSize * 1.2  // 设置行高
  let totalHeight = lines.length * lineHeight

  lines.forEach((line, index) => {
    ctx.fillText(line, (CANVASWIDTH - ctx.measureText(line).width) * 0.5, 
                 (CANVASHEIGHT - totalHeight) * 0.5 + index * lineHeight + lineHeight / 2)
  })
